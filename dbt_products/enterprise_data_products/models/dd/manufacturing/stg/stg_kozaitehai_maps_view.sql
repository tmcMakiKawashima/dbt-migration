...
{% set source_table = source('manufacturing_db_public', 'raw_stg_kozaitehai') %}

with stg_kozaitehai_maps_view as (
  select
    steel_arrangement_id::varchar(15) as steel_arrangement_id, -- 鋼材手配ID
    y_item_no_id::varchar(15) as y_item_no_id, -- Y品番ID
    y_item_no::varchar(10) as y_item_no, -- Y品番
    wh_code::varchar(2) as wh_code, -- 倉庫コード
    organization_id::varchar(15) as organization_id, -- 在庫組織ID
    ...
    notices::varchar(400) as notices, -- 特記事項
...
select * from stg_kozaitehai_maps_view