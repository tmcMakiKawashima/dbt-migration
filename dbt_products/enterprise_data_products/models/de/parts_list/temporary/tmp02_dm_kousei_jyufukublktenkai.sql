with kj1 as (
  ...
  select * from {{ref('tmp01_dm_kousei_jyufukublktenkai')}}
) kj3 as (
  select distinct syasyu, siyoubui, kohin, tyohuku
  from {{ref('tmp01_dm_kousei_jyufukublktenkai')}}
  where tyohuku != ''
) ksc as (
  select * from {{source('engineering_db_public', 'raw_stg_kouseicom')}}
) recursive siyoubui_jyufuku as (
  ...
  select
    ...
    kj2.siyoubui as aite_siyoubui, -- 重複相手先使用部位
  ...
  from kj1
    ...
    left join kj2
      ...
      and left(sj.add_jyufuku, 4) = left(kj2.siyoubui,4)
    ...
    inner join ksc
      ...
      and ksc.comkbn = '15'
      and ksc.jigyoutai = ''
      and ksc.com like '%' || left(sj.aite_siyoubui, 4) || '%'
      )
    left join kj3
    -- 更に重複の下に重複がないか？求める
      on (
...
select distinct 
  ...
  siyoubui as aite_siyoubui, -- 重複相手先使用部位
