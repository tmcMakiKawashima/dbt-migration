...
where(
  case
    when (
      ...
      cast(vsas.sk_y as number(4, 0)) = year(dateadd(month, -2, current_date()))
      and cast(vsas.sk_m as number(2, 0)) = month(dateadd(month, -2, current_date()))
    )
    else false
  end
)
or (
  year(ascv.update_date) = year(dateadd(month, -2, current_date()))
  and month(ascv.update_date) = month(dateadd(month, -2, current_date()))
)