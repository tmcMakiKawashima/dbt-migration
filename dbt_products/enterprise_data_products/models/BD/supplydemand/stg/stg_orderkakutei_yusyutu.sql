...
with stg_orderkakutei_yusyutu as (
select
...
,rtrim(mtflg, ' 　')::varchar(1) as mtflg
...
from {{ref('substr_tsjfa490')}}
where ldts = (select max(ldts) from {{ref('substr_tsjfa490')}})
)
select * exclude(aggkey) from stg_orderkakutei_yusyutu
where aggkey = 1