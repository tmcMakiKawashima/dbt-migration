...
with stg_hostalc_1h_erasure_achievements as (
  select
  ...
  from {{ ref('substr_hostalc_1h_dkae06046602') }}
)
select * from stg_hostalc_1h_erasure_achievements
where ldts = (select max(ldts) from stg_hostalc_1h_erasure_achievements)