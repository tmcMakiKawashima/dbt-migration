...
with stg_hostalc_1g_reject_vehicle as (
  select
  ...
  from {{ ref('substr_hostalc_1g_dkae02072401') }}
)
select * from stg_hostalc_1g_reject_vehicle
where ldts = (select max(ldts) from stg_hostalc_1g_reject_vehicle)