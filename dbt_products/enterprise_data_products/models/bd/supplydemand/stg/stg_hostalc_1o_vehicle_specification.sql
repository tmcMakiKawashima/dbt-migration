...
with stg_hostalc_1o_vehicle_specification as (
  select
  ...
  from {{ ref('substr_hostalc_1o_dkae02072402') }}
)
select * from stg_hostalc_1o_vehicle_specification
where ldts = (select max(ldts) from stg_hostalc_1o_vehicle_specification)