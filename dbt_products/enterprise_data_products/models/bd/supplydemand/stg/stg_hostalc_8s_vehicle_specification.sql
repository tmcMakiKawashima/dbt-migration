...
with stg_hostalc_8s_vehicle_specification as (
  select
  ...
  from {{ ref('substr_hostalc_8s_dkae02072402') }}
)
select * from stg_hostalc_8s_vehicle_specification
where ldts = (select max(ldts) from stg_hostalc_8s_vehicle_specification)