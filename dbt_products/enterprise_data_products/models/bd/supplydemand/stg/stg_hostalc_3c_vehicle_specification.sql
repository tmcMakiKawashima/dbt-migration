...
with stg_hostalc_3c_vehicle_specification as (
  select
  ...
  from {{ ref('substr_hostalc_3c_dkae02072402') }}
)
select * from stg_hostalc_3c_vehicle_specification
where ldts = (select max(ldts) from stg_hostalc_3c_vehicle_specification)