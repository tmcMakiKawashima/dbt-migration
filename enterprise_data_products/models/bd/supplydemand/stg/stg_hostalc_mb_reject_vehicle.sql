with stg_hostalc_mb_reject_vehicle as (
    select
        ...
        ldts -- b層のldts
    from {{ ref('substr_hostalc_mb_dkae02072401') }}
    where ldts = (select max(ldts) from {{ ref('substr_hostalc_mb_dkae02072401') }})
)
select * from stg_hostalc_mb_reject_vehicle