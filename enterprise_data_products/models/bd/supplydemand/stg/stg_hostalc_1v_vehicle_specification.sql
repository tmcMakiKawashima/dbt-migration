...
with stg_hostalc_1v_vehicle_specification as (
  select
  ...
  from {{ ref('substr_hostalc_1v_dkae02072402') }}
)
select * from stg_hostalc_1v_vehicle_specification
where ldts = (select max(ldts) from stg_hostalc_1v_vehicle_specification)