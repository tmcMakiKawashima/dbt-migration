...
with stg_hostalc_2q_vehicle_specification as (
  select
  ...
  from {{ ref('substr_hostalc_2q_dkae02072402') }}
)
select * from stg_hostalc_2q_vehicle_specification
where ldts = (select max(ldts) from stg_hostalc_2q_vehicle_specification)