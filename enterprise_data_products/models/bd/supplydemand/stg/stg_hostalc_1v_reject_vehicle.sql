...
with stg_hostalc_1v_reject_vehicle as (
  select
  ...
  from {{ ref('substr_hostalc_1v_dkae02072401') }}
)
select * from stg_hostalc_1v_reject_vehicle
where ldts = (select max(ldts) from stg_hostalc_1v_reject_vehicle)