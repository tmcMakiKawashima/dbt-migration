with stg_qm_reject_vehicle_hostalc as (
    select
        ...
        ldts -- b層のldts
    from {{ ref('substr_hostalc_qm_dkae02072401') }}
    where ldts = (select max(ldts) from {{ ref('substr_hostalc_qm_dkae02072401') }})
)
select * from stg_qm_reject_vehicle_hostalc