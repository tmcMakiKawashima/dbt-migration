with stg_hostalc_pb_virtual_line as (
    select
        ...
        ldts -- B層取込日時
    from {{ ref('substr_hostalc_pb_dkae02072301') }}
    where ldts = (select max(ldts) from {{ ref('substr_hostalc_pb_dkae02072301') }})
)
select * from stg_hostalc_pb_virtual_line