with stg_pb_erasure_achievements_hostalc as (
    select
        ...
        ldts -- b層のldts
    from {{ ref('substr_hostalc_pb_dkae06046602') }}
    where ldts = (select max(ldts) from {{ ref('substr_hostalc_pb_dkae06046602') }})
)
select * from stg_pb_erasure_achievements_hostalc