with stg_hostalc_vm_virtual_line as (
    select
        ...
        ldts -- b層のldts
    from {{ ref('substr_hostalc_vm_dkae02072301') }}
    where ldts = (select max(ldts) from {{ ref('substr_hostalc_vm_dkae02072301') }})
)
select * from stg_hostalc_vm_virtual_line