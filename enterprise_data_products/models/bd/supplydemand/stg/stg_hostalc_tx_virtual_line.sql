with stg_hostalc_tx_virtual_line as (
    select
    ...
    from {{ ref('substr_hostalc_tx_dkae02072301') }}
    where ldts = (select max(ldts) from {{ ref('substr_hostalc_tx_dkae02072301') }})
)
select * from stg_hostalc_tx_virtual_line