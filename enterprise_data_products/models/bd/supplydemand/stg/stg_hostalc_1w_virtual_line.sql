...
with stg_hostalc_1w_virtual_line as (
  select
  ...
  from {{ ref('substr_hostalc_1w_dkae02072301') }}
)
select * from stg_hostalc_1w_virtual_line
where ldts = (select max(ldts) from stg_hostalc_1w_virtual_line)