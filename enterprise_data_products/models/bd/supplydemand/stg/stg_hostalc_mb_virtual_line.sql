with stg_hostalc_mb_virtual_line as (
    select
        ...
        ldts -- b層のldts
    from {{ ref('substr_hostalc_mb_dkae02072301') }}
    where ldts = (select max(ldts) from {{ ref('substr_hostalc_mb_dkae02072301') }})
)
select * from stg_hostalc_mb_virtual_line