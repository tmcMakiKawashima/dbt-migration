with stg_vm_erasure_achievements_hostalc as (
    select
    ...
    where ldts = (select max(ldts) from {{ ref('substr_hostalc_vm_dkae06046602') }})
)
select * from stg_vm_erasure_achievements_hostalc