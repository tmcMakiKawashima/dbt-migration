with stg_pb_production_progress_hostalc as (
    select
        ...
        ldts -- b層のldts
    from {{ ref('substr_hostalc_pb_dkae06047201') }}
    where ldts = (select max(ldts) from {{ ref('substr_hostalc_pb_dkae06047201') }})
)
select * from stg_pb_production_progress_hostalc