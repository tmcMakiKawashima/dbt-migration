...
with stg_hostalc_1i_vehicle_specification as (
  select
  ...
  from {{ ref('substr_hostalc_1i_dkae02072402') }}
)
select * from stg_hostalc_1i_vehicle_specification
where ldts = (select max(ldts) from stg_hostalc_1i_vehicle_specification)