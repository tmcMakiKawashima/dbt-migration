...
with stg_hostalc_1h_reject_vehicle as (
  select
  ...
  from {{ ref('substr_hostalc_1h_dkae02072401') }}
)
select * from stg_hostalc_1h_reject_vehicle
where ldts = (select max(ldts) from stg_hostalc_1h_reject_vehicle)