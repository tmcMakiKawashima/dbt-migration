with stg_hostalc_pb_vehicle_specification as (
    select
    ...
    from {{ ref('substr_hostalc_pb_dkae02072402') }}
    where ldts = (select max(ldts) from {{ ref('substr_hostalc_pb_dkae02072402') }})
)
select * from stg_hostalc_pb_vehicle_specification