...
with stg_hostalc_1v_production_progress as (
  select
  ...
  from {{ ref('substr_hostalc_1v_dkae06047201') }}
)
select * from stg_hostalc_1v_production_progress
where ldts = (select max(ldts) from stg_hostalc_1v_production_progress)