with stg_hostalc_mb_vehicle_specification as (
    select
        ...
        ldts -- b層のldts
    from {{ ref('substr_hostalc_mb_dkae02072402') }}
    where ldts = (select max(ldts) from {{ ref('substr_hostalc_mb_dkae02072402') }})
)
select * from stg_hostalc_mb_vehicle_specification