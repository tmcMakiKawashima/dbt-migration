...
with stg_hostalc_1v_virtual_line as (
  select
  ...
  from {{ ref('substr_hostalc_1v_dkae02072301') }}
)
select * from stg_hostalc_1v_virtual_line
where ldts = (select max(ldts) from stg_hostalc_1v_virtual_line)