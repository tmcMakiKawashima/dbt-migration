...
with stg_hostalc_pd_vehicle_specification as (
  select
  ...
  from {{ ref('substr_hostalc_pd_dkae02072402') }}
  where ldts = (select max(ldts) from {{ ref('substr_hostalc_pd_dkae02072402') }})
)
select * from stg_hostalc_pd_vehicle_specification