with stg_vm_vehicle_specification_hostalc as (
    select
    ...
    where ldts = (select max(ldts) from {{ ref('substr_hostalc_vm_dkae02072402') }})
)
select * from stg_vm_vehicle_specification_hostalc