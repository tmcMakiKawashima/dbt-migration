...
with stg_hostalc_1f_reject_vehicle as (
  select
  ...
  from {{ ref('substr_hostalc_1f_dkae02072401') }}
)
select * from stg_hostalc_1f_reject_vehicle
where ldts = (select max(ldts) from stg_hostalc_1f_reject_vehicle)