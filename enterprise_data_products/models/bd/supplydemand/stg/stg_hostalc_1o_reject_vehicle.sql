...
with stg_hostalc_1o_reject_vehicle as (
  select
  ...
  from {{ ref('substr_hostalc_1o_dkae02072401') }}
)
select * from stg_hostalc_1o_reject_vehicle
where ldts = (select max(ldts) from stg_hostalc_1o_reject_vehicle)