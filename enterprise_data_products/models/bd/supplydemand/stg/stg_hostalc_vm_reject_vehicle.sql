...
with stg_hostalc_vm_reject_vehicle as (
  select
  ...
  from {{ ref('substr_hostalc_vm_dkae02072401') }}
  where ldts = (select max(ldts) from {{ ref('substr_hostalc_vm_dkae02072401') }})
)
select * from stg_hostalc_vm_reject_vehicle