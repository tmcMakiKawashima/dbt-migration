with stg_hostalc_mb_erasure_achievements as (
    select
        ...
        ldts -- b層のldts
    from {{ ref('substr_hostalc_mb_dkae06046602') }}
    where ldts = (select max(ldts) from {{ ref('substr_hostalc_mb_dkae06046602') }})
)
select * from stg_hostalc_mb_erasure_achievements