with stg_3c_erasure_achievements_hostalc as (
    select
    ...
    where ldts = (select max(ldts) from {{ ref('substr_hostalc_3c_dkae06046602') }})
)
select * from stg_3c_erasure_achievements_hostalc