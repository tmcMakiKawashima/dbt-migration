...
with stg_hostalc_tx_erasure_achievements as (
  select
  ...
  from {{ ref('substr_hostalc_tx_dkae06046602') }}
)
select * from stg_hostalc_tx_erasure_achievements
where ldts = (select max(ldts) from stg_hostalc_tx_erasure_achievements)