with stg_hostalc_pd_erasure_achievements as (
    select
    ...
    from {{ ref('substr_hostalc_pd_dkae06046602') }}
    where ldts = (select max(ldts) from {{ ref('substr_hostalc_pd_dkae06046602') }})
)
select * from stg_hostalc_pd_erasure_achievements