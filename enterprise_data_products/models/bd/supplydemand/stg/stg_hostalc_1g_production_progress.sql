...
with stg_hostalc_1g_production_progress as (
  select
  ...
  from {{ ref('substr_hostalc_1g_dkae06047201') }}
)
select * from stg_hostalc_1g_production_progress
where ldts = (select max(ldts) from stg_hostalc_1g_production_progress)