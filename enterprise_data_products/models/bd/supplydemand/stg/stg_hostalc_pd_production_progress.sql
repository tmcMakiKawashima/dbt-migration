...
with stg_hostalc_pd_production_progress as (
  select
  ...
  from {{ ref('substr_hostalc_pd_dkae06047201') }}
  where ldts = (select max(ldts) from {{ ref('substr_hostalc_pd_dkae06047201') }})
)
select * from stg_hostalc_pd_production_progress