...
with
    tmp_get_union as (
        select
            ...
            sketa_cd,
            skigo_cd
        from {{ source('vinhis_db_vinspec','raw_dm_vinhis_specification_spec_allsalecar') }}
        where
            coalesce(trim(sketa_cd), '') <> ''
            and coalesce(trim(skigo_cd), '') <> ''
    ),
    ...
    tmp_join_siyo4 as (
        ...
        from tmp_get_union as a
        left join tmp_get_siyouhenkan_latest as b
            on
                trim(a.syasyu) = trim(b.syasyu)
                and trim(a.sketa_cd) = trim(b.s1keta)
                and trim(a.skigo_cd) = trim(b.s1kigo)
    ),
