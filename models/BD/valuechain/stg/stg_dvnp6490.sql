...
with stg_dvnp6490 as (
    select
        ...
        rtrim(REMARK10,' 　')::VARCHAR(10) as REMARK10,  -- 英数字
        ...
        to_decimal(IFF(rtrim(NOKHNKCT) = '',0,rtrim(NOKHNKCT)))::DECIMAL(3) as NOKHNKCT,  -- 数量／金額／数値
        ...
        rtrim(HNKAITOYMD,' 　')::VARCHAR(8) as HNKAITOYMD,  -- 英数字
        ...
        to_decimal(IFF(rtrim(KGIHIRITUF) = '',0,rtrim(KGIHIRITUF)))::DECIMAL(4) as KGIHIRITUF,  -- 英数字
        rtrim(FILLER2,' 　')::VARCHAR(5) as FILLER2,  -- 英数字
        rtrim(HKANRI,' 　')::VARCHAR(4) as HKANRI,  -- 英数字
        LDTS,  --B層のLDTS
        RANK() over (partition by ORDRKEY, JURRSYMD, TANSKKEY order by LDTS desc) aggkey
