with
    ...
    stg_hinbanmeisyomaster as (
        select
            parts_cd, -- 品番コード
            parts_name -- 品番名称
        from {{ source('customerservice_db_public', 'stg_hinbanmeisyomaster') }} -- 整備名称情報マスタ
    )
select
    ...
    tmp10_dm_vinhis_buhinreki.meisaino as nyuko_detail_no, -- 明細行番号
    tmp10_dm_vinhis_buhinreki.hinban as parts_cd, -- 品番コード
    stg_hinbanmeisyomaster.parts_name as parts_name, -- 品番名称
    tmp10_dm_vinhis_buhinreki.buhinsuryo as parts_amount -- 部品数量
from tmp10_dm_vinhis_buhinreki
left outer join stg_hinbanmeisyomaster on tmp10_dm_vinhis_buhinreki.hinban = stg_hinbanmeisyomaster.parts_cd
